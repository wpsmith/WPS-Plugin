	...
	class ExtendPlugin extends Singleton {
		...
		public function __construct( $plugin, $root_file, $min_version = null, $text_domain = null, $custom_plugins_folder = WPMU_PLUGIN_DIR ) {
			...
			if ( $custom_plugins_folder ) {
				...
				$this->mu = get_plugins( $custom_plugins_folder );
			}
			...
			if ( 'plugins.php' === basename( $_SERVER['PHP_SELF'] ) && ! ( defined( 'WP_CLI' ) && WP_CLI ) ) {
				...
				add_filter( 'plugin_action_links_' . plugin_basename( $root_file ), array(
					$this,
					'plugin_action_links'
				), 10, 4 );
				add_filter( 'network_admin_plugin_action_links_' . plugin_basename( $root_file ), array(
					$this,
					'plugin_action_links'
				), 10, 4 );
